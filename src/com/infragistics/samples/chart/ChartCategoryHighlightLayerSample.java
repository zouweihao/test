...
public class ChartCategoryHighlightLayerSample extends SampleChart {

	public ChartCategoryHighlightLayerSample(Activity activity,
			SampleInfo info, boolean useLegend, boolean smallData) {
		super(activity, info, useLegend);

		CreateHorizontalCategorySeries(info.seriesClass, smallData);
			 	
		CategoryHighlightLayer chl = new CategoryHighlightLayer();
		chl.setTransitionDuration(500);
		chl.setUseInterpolation(false);
		//chl.setBrush(Brushes.BlueTransparent);
		 
		chl.setBrush(Brushes.GetBrush(Colors.BlueLight, 0.25));
	...
	private void CreateHorizontalCategorySeries(Class<?> sampleType, final boolean smallData) {
		...
		ChartToolTipAdapter tip = new ChartToolTipAdapter(context) {
			...
			public View getView(ChartDataContext chartContext, View existingView, CustomContentUpdateInfo updateInfo) {
				...
				if (lin == null) {
					lin = new android.widget.LinearLayout(activity);
	...
	protected String getDescription() {
		return "This sample demonstrates the Category Highlight Layer that targets a category axis. It can be configured by setting its color, outline, thickness, opacity.";
	}
}
